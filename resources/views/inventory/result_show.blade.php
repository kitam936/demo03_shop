    ...
    <x-slot name="header">
        ...
        <div class="md:flex">
            ...
            <div class="pl-2 mt-2 ml-4 ">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('analysis_index') }}'" >Menu</button>
            </div>
            @if($user->shop_id >100)
            <div class="pl-2 mt-2 ml-4 ">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('inventory_result_index') }}'" >棚卸リスト</button>
            </div>
            </div>
                                ...
                                <div class="p-2 w-full mx-auto">
                                    ...
                                    <div class="flex">
                                        ...
                                        <div class="relative ml-2">
                                            ...
                                            <div  id="total_pcs" name="total_pcs" value="{{$inventory_h->total_pcs}}" class="h-8 text-sm w-24 bg-gray-100 bg-opacity-50 border rounded focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out">{{$inventory_h->total_pcs}}枚
                                            </div>
                                        </div>
                                        <div class="relative ml-2">
                                            <label for="user_name" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">作業者</label>
                                            <div  id="user_name" name="user_name" value="{{$inventory_h->name}}" class="h-8 text-sm w-32 bg-gray-100 bg-opacity-50 border rounded focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out">{{$inventory_h->name}}
                                            </div>
                                        </div>
                                        <div class="relative ml-2">
                                            <label for="status" class="leading-7 text-sm  text-gray-800 dark:text-gray-200 ">Status</label>
                                            <div  id="status" name="status" value="{{$inventory_h->status_name}}" class="h-8 text-sm w-24 bg-gray-100 bg-opacity-50 border rounded focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-8 transition-colors duration-200 ease-in-out">{{$inventory_h->status_name}}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="relative ml-2">
            ...
            <table class="md:w-2/3 bg-white table-auto w-full text-center whitespace-no-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="w-3/12 md:3/12 text-sm md:px-4 py-1 text-center">{{ $inventory_f->hinban_id }}</td>
                        <td class="w-3/12 md:3/12 text-sm md:px-4 py-1 text-center">{{ Str::limit($inventory_f->hinban_name,10) }}</td>
                        <td class="w-2/12 md:2/12 text-sm md:px-4 py-1 text-center">{{ $inventory_f->col_id }}</td>
                        <td class="w-2/12 md:2/12 text-sm md:px-4 py-1 text-center">{{ $inventory_f->size_id }}</td>
                        <td class="w-2/12 md:2/12 text-sm md:px-4 py-1 text-center">{{ $inventory_f->f_pcs }}</td>
                    </tr>
                    @endforeach
