<x-app-layout>
    <x-slot name="header">
        ...
        <div class="md:flex">
            ...
            <div class="pl-2 mt-2 ml-4 ">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('inventory_index') }}'" >棚卸入力状況</button>
            </div>
            </div>
            <div class="flex">
            <div class="pl-2 ml-4 mt-2 md:ml-4 md:mt-2">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-green-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-green-700 rounded " onclick="location.href='{{ route('inventory_scan') }}'" >棚卸開始</button>
            </div>
            @if($user->role_id < 3)
            @if($dl_new)
            <div class="pl-2 ml-4 mt-2 md:ml-4 md:mt-2">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-blue-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-blue-700 rounded " onclick="location.href='{{ route('inventory_dl_all') }}'" >New一括DL</button>
            </div>
            @else
            ...
            <table class="md:w-2/3 bg-white table-auto w-full text-center whitespace-no-wrap">
               <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="w-1/14 md:px-4 py-1 text-center"> {{ $inventory_h->id }}</td>
                        <td class="w-3/14 pl-2 text-sm md:px-4 py-1 text-center"> <a href="{{ route('inventory_result_show',['id'=>$inventory_h->id]) }}" class="w-20 h-8 text-indigo-500 ml-2 "  >{{\Carbon\Carbon::parse($inventory_h->inventory_date)->format("y/m/d")}}</a></td>
                        ...
                        <td class="w-3/14 pl-2 text-red-500 text-sm md:px-4 py-1 text-center"> {{ $inventory_h->status_name }}</td>
                        @else
