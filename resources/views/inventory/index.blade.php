<x-app-layout>
    ...
    <div  class="mt-2 ml-4 h-6 text-left text-sm w-60 bg-gray-100 rounded bg-opacity-50 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 px-3 leading-6 transition-colors duration-200 ease-in-out">Name: {{ Auth::user()->name }}
    </div>

    <div class="ml-4 flex mb-4 md:mb-0 md:ml-4">
        <div class="ml-4 mt-2 md:mt-0 md:ml-8">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('analysis_index') }}'" >Menu</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('inventory_result_index') }}'" >棚卸リスト</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-green-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-green-600 rounded " onclick="location.href='{{ route('inventory_scan') }}'" >棚卸開始</button>
        </div>
    </div>
    ...
    @if($h_exist || $s_exist)
    <div class="ml-4 text-ml text-red-500">
    ...
    <form method="POST" action="{{ route('inventory_complete') }}" class="mt-2">
        @csrf
        <label>memo：<input type="text" name="memo" style="width:300px;" placeholder="ロケーション等の注記事項を入力" class="rounded w-50" ></label>
        <button type="submit" class="mt-4 ml-0 md:ml-4 w-24 h-8 text-center text-sm text-white bg-pink-500 binventory-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded " onclick="return confirm('棚卸を確定してよろしいですか？')">棚卸確定</button>
    </form>
    </div>
    @endif
                    ...
                    <td class="w-5/16 md:px-2 py-1" >
                        <form method="POST" action="{{ route('inventory_update', $work->id) }}">
                            ...
                            <div class="flex ml-1">
                                ...
                                <input readonly value="{{ substr($work->raw_cd,0,6) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:120px; font-variant-numeric:tabular-nums">
                                @else
                                ...
                                <input readonly value="{{ substr($work->raw_cd,6,2) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:60px; font-variant-numeric:tabular-nums">
                                <input readonly value="{{ substr($work->raw_cd,8,2) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:60px; font-variant-numeric:tabular-nums">
                                @else
            ...
            <tr>
                <td class="w-5/16 md:px-2 py-1" >
                    <form method="POST" action="{{ route('inventory_update', $work->id) }}">
                        @csrf
                        <div class="flex ml-1">
                            <input  type="hidden" name="raw_cd" value="{{ $work->raw_cd }}" class="rounded mr-2" style="width:160px; font-variant-numeric:tabular-nums">
                            @if(is_null($work->sku_ck))
                            <input type="hidden" value="{{ ($work->id) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:120px; font-variant-numeric:tabular-nums">
                            <input readonly value="{{ substr($work->raw_cd,0,6) }}-{{ substr($work->raw_cd,6,2)}}-{{ substr($work->raw_cd,8,2) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:150px; font-variant-numeric:tabular-nums">
                            @else
                            <input type="hidden" value="{{ ($work->id) }}" class="rounded mr-2 bg-red-400 text-ml" style="width:120px; font-variant-numeric:tabular-nums">
                            <input readonly value="{{ substr($work->raw_cd,0,6) }}-{{ substr($work->raw_cd,6,2)}}-{{ substr($work->raw_cd,8,2) }}" class="rounded mr-2 bg-gray-200 text-ml " style="width:150px; font-variant-numeric:tabular-nums">
                            @endif
