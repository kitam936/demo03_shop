    ...
    <x-slot name="header">
        ...
        <div class="flex">
            <div class="flex">
                ...
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-blue-500 border-0 py-1 px-2 focus:outline-none hover:bg-blue-700 rounded " onclick="location.href='{{ route('sales_product_reset') }}'" >選択リセット</button>
                </div>
            </div>
        </div>
        ...
        <form method="get" action="{{ route('sales_product')}}" class="mt-4">
            ...
            <div>
                <label for="type3" class="mr-0 leading-7 text-sm  text-gray-800 ">集計区分切替　</label>
                <select id="type3" name="type3" class="w-40 h-8 rounded text-sm pt-1 border mr-6 mb-2" type="text">
                    <option value="" @if(\Request::get('type3') == '0') selected @endif >選択して画像表示</option>
            ...
            <div class="flex md:ml-0 mb-2">
                <div>
                <label for="YM1" class="mr-2 leading-7 text-sm  text-gray-800 ">期間(年月)</label>
                <select class="md:ml-6 w-36 h-8 rounded text-sm pt-1" id="YM1" name="YM1" type="number" class="border">
               ...
               <span class="items-center text-sm mt-4 mr-2 ml-0 md:mt-2 text-gray-800 leading-tight" >　　～</span>
                <div>
                 <label for="YM2" class="mr-0 leading-7 text-sm  text-gray-800 ">＊初期表示は当月</label>
            ...
            <div class="flex md:ml-0">
                ...
                <select class="md:ml-8 w-28 h-8 rounded text-sm pt-1 border mb-2 mr-6" id="co_id" name="co_id" >
                ...
                <select class="md:ml-2 w-32 h-8 rounded border text-sm items-center pt-1" id="sh_id" name="sh_id" >
            ...
            <div class="flex">
                <div>
                <label for="brand_code" class="mr-2 leading-7 text-sm  text-gray-800 ">Brand指定</label>
                <select class="md:ml-6 w-28 h-8 rounded text-sm pt-1 border mb-2 mr-6 " id="brand_code" name="brand_code" type="number" >
        ...
        <div class="md:w-full sm:px-4 lg:px-4 border">
            <table class="mx-auto table-auto bg-white w-full text-center whitespace-no-wrap">
                <thead >
                <tr>
                    <th class="w-2/12 md:px-2 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">品番/SKU</th>
                    {{-- <th class="w-3/12 md:px-4 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">品番</th> --}}
                    <th class="w-4/12 md:px-2 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">品名</th>
                    <th class="w-2/12 md:px-2 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">売価</th>
                    <th class="w-1/12 md:px-2 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">数</th>
                    <th class="w-3/12 md:px-2 py-1 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">商品画像</th>
                </tr>
                </thead>
                ...
                <tr>
                    <td class="w-2/12 md:px-2 py-1 text-sm  text-indigo-700"><a href="{{ route('product_show',['hinban'=>$data->hinban_id]) }}" >{{ $data->code }}</a></td>
                    {{-- <td class="w-3/12 md:px-4 py-1 text-sm">{{ $data->hinban_id }}</td> --}}
                    <td class="w-4/12 md:px-2 py-1 text-sm">{{ Str::limit($data->hinban_name, 20, '...')}}</td>
                    <td class="w-2/12 pr-2 md:px-1 text-sm py-1 text-center"><span style="font-variant-numeric:tabular-nums"> {{ number_format(round($data->m_price))}}</span></td>
                    <td class="w-1/12 pr-1 md:px-1 text-sm py-1 text-center"><span style="font-variant-numeric:tabular-nums"> {{ number_format(round($data->pcs_total))}}</span></td>
                    ...
                    <td class="w-3/12 md:px-0 py-0">
                        <div class="ml-0 md:ml-16 items-right w-full md:w-2/3 ">
                            ...
                            @if(\Request::get('type3') == "h" )
                                @if(($data->filename))
                                <a href="{{ route('image_show2',['hinban'=>$data->hinban_id]) }}">
                                    <x-image-thumbnail :filename="$data->filename"  />
                                </a>   
                                @endif   
                                @if(!($data->filename))                        
