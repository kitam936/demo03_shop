<x-app-layout>
    ...
    <div class="ml-2 flex mb-4 md:mb-0 md:ml-4">
        ...
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('pos_result_index') }}'" >売上Data</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-green-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-green-600 rounded " onclick="location.href='{{ route('pos_scan') }}'" >SCAN開始</button>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('pos_complete') }}" class="mt-2">
        @csrf
        <label>memo：<input type="text" name="memo" style="width:300px;" placeholder="注記事項を入力" class="rounded w-50" ></label>
        <button type="submit" class="mt-4 ml-0 md:ml-4 w-24 h-8 text-center text-sm text-white bg-pink-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded " onclick="return confirm('売上を確定してよろしいですか？')">確定</button>
    </form>
    </div>
    @endif
...
<div class="mt-4">
    <h3 class="flex ml-4 font-semibold text-ml text-indigo-700 leading-tight mt-4">
        ...
        <div class="flex relative ml-10">
            {{-- <label for="total_gaku" class="leading-7 text-ml text-indigo-700 ">金額</label> --}}
            @if($works_total)
