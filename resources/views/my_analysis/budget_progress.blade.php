    ...
    <x-slot name="header">
        ...
        <div class="flex">
            ...
            <div class="pl-2 mt-2  ml-8 ">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-blue-500 border-0 py-1 px-2 focus:outline-none hover:bg-blue-700 rounded " onclick="location.href='{{ route('my_budget_progress_reset') }}'" >選択リセット</button>
            </div>
        </div>
        ...
        <div class="ml-0 mt-3 py-0 md:w-2/3 border">
            <div class="md:flex w-full  sm:px-0 lg:px-0 border mt-0 ml-0">
                ...
                <div class='md:w-2/3 pl-0 border bg-gray-100 h-6 text-sm'>
                    @if($bg_total->total>0)
                    　予算比　：{{ number_format(floor(($total->total/$bg_total->total)*100))}}％
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="md:w-2/3 sm:px-4 lg:px-4 border">
            <table class="mx-auto table-auto bg-white w-full text-center whitespace-no-wrap">
                ...
                <tr>
                    ...
                    @if($data->bg_total>0)
                    <td class="w-3/12 pr-8 md:px-8 py-1 text-sm text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format(floor(($data->total/$data->bg_total)*100))}} %</span></td>
                    @else
