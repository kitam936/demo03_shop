...
<x-app-layout>
    <x-slot name="header">
        ...
        <div class="ml-0 mt-2 md:mt-0 md:ml-8">
            <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('shop_index') }}'" >shop一覧</button>
        </div>

        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('report_create',['shop'=>$shop->id]) }}'" >新規Report</button>
        </div>
        </div>
