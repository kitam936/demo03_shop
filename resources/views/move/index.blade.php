<x-app-layout>
    ...
    <div class="ml-2 flex mb-4 md:mb-0 md:ml-4">
        ...
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('move_result_index') }}'" >Dataリスト</button>
        </div>
        <div class="ml-4 mt-2 md:ml-4 md:mt-0">
            <button type="button" class="w-24 h-8 text-center text-sm text-white bg-green-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-green-600 rounded " onclick="location.href='{{ route('move_scan') }}'" >SCAN開始</button>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('move_complete') }}" class="mt-2">
        ...
        <label for="to_shop_id" class="mr-5 leading-7 text-sm  text-gray-800 ">着店指定</label>
            <select class="w-32 h-8 rounded border text-sm items-center pt-1" id="to_shop_id" name="to_shop_id" >
                <option value="106" @if(\Request::get('to_shop_id') == '0') selected @endif >ジジョン神田倉庫</option>
        ...
        <div class="mt-0">
        <label>memo：</label>
        <input type="text" name="memo" style="width:300px;" placeholder="注記事項を記入" class="rounded w-50" >
        <button type="submit" class="mt-4 ml-0 md:ml-4 w-24 h-8 text-center text-sm text-white bg-pink-500 bpos-0 py-1 px-2 focus:outline-none hover:bg-pink-700 rounded " onclick="return confirm('Dataを確定してよろしいですか？')">確定</button>
        </div>
    </form>
    </div>
    @endif
        ...
        <table class="ml-1 bg-white text-left table-auto whitespace-no-wrap">
            ...
            <thead>
                <tr>
                    ...
                    <th class="w-3/16 pr-8 md:pr-0 py-1 text-left">　　品番-色-サイズ　　　　数</th>
                    ...
                    <td class="w-5/16 md:px-2 py-1" >
                        <form method="POST" action="{{ route('move_update', $work->id) }}">
                            ...
                            <div class="flex ml-1">
                                ...
                                @endif
                                <input type="hidden" name="move_price2" value="{{ $work->move_price }}"  class="rounded mr-2 text-ml " style="width:80px; font-variant-numeric:tabular-nums">
