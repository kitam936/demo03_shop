    ...
    <x-slot name="header">
        ...
        <div class="md:flex md:ml-8 ">
            ...
            <div class="md:ml-10 mb-2">
                <button type="button" class="w-32 h-8 text-center text-sm text-white bg-indigo-500 border-0 py-1 px-2 focus:outline-none hover:bg-indigo-700 rounded " onclick="location.href='{{ route('report_detail',['report'=>$comment->report_id]) }}'" >Report詳細</button>
            </div>
        ...
        <div class="md:ml-2 mb-2 md:mb-0">
            <button type="button" onclick="location.href='{{ route('comment_edit',['comment'=>$comment->id])}}'" class="w-32 h-8 text-center text-sm text-white bg-green-500 border-0 py-1 px-2 focus:outline-none hover:bg-green-700 rounded ">編集</button>
        </div>
        <form id="delete_{{$comment->id}}" method="POST" action="{{ route('comment_destroy',['comment'=>$comment->id]) }}">
            ...
            <div class="ml-0 mt-2 md:ml-4 md:mt-0">
                <div class="w-32 h-8 text-center text-sm text-white bg-red-500 border-0 py-1 px-2 focus:outline-none hover:bg-red-700 rounded ">
