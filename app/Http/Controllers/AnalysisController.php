...
class AnalysisController extends Controller
{
    public function analysis_index()
    {
        ...
        $dl_new_order = DB::table('orders') //未ＤＬ判定用
        ->where('orders.status',1)
        // ->where('orders.to_shop_id',101)
        ->exists();

        $dl_new_d = DB::table('move_headers') //未ＤＬ判定用
        ->where('move_headers.status_id',1)
        // ->where('move_headers.to_shop_id','<>',106)
        ->exists();

        $dl_new_pos = DB::table('pos_headers') //未ＤＬ判定用
        ->where('pos_headers.status_id',1)
        // ->where('orders.to_shop_id',101)
        ->exists();

        $dl_new_inv = DB::table('inventory_headers') //未ＤＬ判定用
        ->where('inventory_headers.status_id',1)
        // ->where('orders.to_shop_id',101)
        ->exists();

        // dd($logIn_user,$my_shop);
        return view('analysis.analysis_menu',compact('logIn_user','my_shop','dl_new_k','dl_new_order','dl_new_d','dl_new_pos','dl_new_inv'));
    }
