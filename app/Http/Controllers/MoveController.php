...
class MoveController extends Controller
{
    ...
    public function result_index() {
        ...
        ->join('move_statuses','move_statuses.id','move_headers.status_id')
        ->where('move_headers.to_shop_id',106)
        ->groupBy('move_details.move_header_id','move_headers.from_shop_id','move_headers.to_shop_id','move_headers.status_id','move_statuses.status_name','shops.shop_name','to_shops.shop_name','users.name','move_headers.move_date','move_headers.created_at')
        ->selectRaw('move_details.move_header_id as id,move_headers.move_date,move_headers.from_shop_id,move_headers.to_shop_id,move_headers.status_id,move_statuses.status_name,shops.shop_name,to_shops.shop_name as to_shop_name,users.name,sum(move_details.pcs) as pcs,move_headers.created_at')
        ->orderBy('move_header_id','desc')
        ->paginate(50);
        ...
        $dl_new_k = DB::table('move_headers') //未ＤＬ判定用
        ->where('move_headers.status_id',1)
        ->where('move_headers.to_shop_id',106)
        ->exists();
        ...
        return view('move.result_index', compact('move_hs','user','move_hs2','move_ks','dl_new','dl_new_k'));
    }
