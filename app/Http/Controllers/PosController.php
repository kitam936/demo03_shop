    ...
    public function complete(Request $request)
{
    ...
    public function download($id)
    {
        ...
        ->groupBy('pos_headers.id','pos_headers.pos_date','pos_headers.shop_id','shops.shop_name','users.name','pos_details.sku_id','skus.hinban_id','skus.col_id','skus.size_id','pos_details.price','pos_headers.memo')
        ->selectRaw('pos_headers.id,pos_headers.pos_date,pos_headers.shop_id,shops.shop_name,users.name,pos_details.sku_id,skus.hinban_id,skus.col_id,skus.size_id,pos_details.price,sum(pos_details.pcs) as pcs,sum(pos_details.pcs * pos_details.price) as uriage,pos_headers.memo')
        ->orderBy('sku_id','asc')
        ->distinct()
        ...
        $csvHeader = [
            'id','date','shop_id','shop_name','staff_name','sku_id','hinban_id','col_id','size_id','price','pcs','uriage','memo'];
        ...
        $response->headers->set('Content-Type', 'text/csv');
        $response->headers->set('Content-Disposition', 'attachment; filename="POS実績Data_' . $timestamp . '.csv"');
    ...
    public function download_iyc()
    {
        ...
        $timestamp = date('ymd_His');
        $response->headers->set('Content-Type', 'text/csv');
        $response->headers->set('Content-Disposition', 'attachment; filename="IYC_POS実績Data_' . $timestamp . '.csv"');
    ...
    public function download_izc()
    {
        ...
        $timestamp = date('ymd_His');
        $response->headers->set('Content-Type', 'text/csv');
        $response->headers->set('Content-Disposition', 'attachment; filename="IZC_POS実績Data_' . $timestamp . '.csv"');
    ...
    public function result_index() {
        ...
        $dl_new_izc = DB::table('pos_headers') //未ＤＬ判定用
        ->join('shops','shops.id','pos_headers.shop_id')
        ->where('pos_headers.status_id',1)
        ->where('shops.company_id',5500)
        ->exists();

        $dl_new_iyc = DB::table('pos_headers') //未ＤＬ判定用
        ->join('shops','shops.id','pos_headers.shop_id')
        ->where('pos_headers.status_id',1)
        ->where('shops.company_id',5200)
        ->exists();
        ...
        return view('pos.result_index', compact('pos_hs','user','pos_hs2','dl_new','dl_new_izc','dl_new_iyc'));
    }
