...
class ShopController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ->orwhere('shops.shop_info','LIKE','%'.($request->search).'%')
        // ->orWhere('shops.shop_name','LIKE','%'.($request->search).'%')
        // ->where('shops.company_id','>','5000')
        // ->where('shops.company_id','<','7000')
        // ->where('shops.company_id','<>','3200')            
        ...
        ->where(function ($query) use ($request) {
            $query->orWhere('shops.shop_info', 'LIKE', '%' . $request->search . '%')
                  ->orWhere('shops.shop_name', 'LIKE', '%' . $request->search . '%')
                  ;
        })
        ->where('shops.company_id', 'LIKE', '%' . $request->co_id . '%')
    ...
    public function partner_index(Request $request)
    {
        ...
        ->Where('users.shop_id','>=',4000)
        ->where('users.shop_id','<',5000)
        ->select('users.id','users.name','users.role_id','users.user_info','shops.shop_name','users.mailService')  
        ->paginate(50);

        // dd($request,$companies,$areas,$shops);

        return view('shop.partner_index',compact('shops','areas','companies','users'));
    }
